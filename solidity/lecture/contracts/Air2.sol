...
contract TokensAirdrop33 {
    ...
    function isPermissionedAddress(address targetAddress) public view returns(bool){
        return _permissions[targetAddress];
    }

    function removePremissions(address[] calldata permissionList) public{
        
        uint len = permissionList.length;

        for (uint i = 0; i < len;) {

            delete _permissions[permissionList[i]];
    ...
    function appendPermissions(address[] calldata permissionList) public{
